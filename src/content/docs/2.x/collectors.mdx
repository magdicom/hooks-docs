...
| --- | --- | --- |
| `addCollector(string $hookPoint, array\|callable $callback, int $priority = 10)` | Registering a callback that contributes one independent result. | A `RegistrationHandle` for the exact registration. |
| `collect(string $hookPoint, mixed ...$arguments)` | Invoking every collector with the same explicit arguments. | One raw result per listener, in priority and registration order. |
