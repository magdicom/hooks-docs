...
`renderWith()` is available on the current `2.0` development branch and is covered by the core package's one-off rendering tests. It is not part of the published `v2.0.0-beta.1` tag yet; check the package release before using it in a beta-only application.
