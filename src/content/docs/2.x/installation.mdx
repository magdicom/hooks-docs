...
The released core tag requires PHP `^8.2` and has no runtime dependencies. The released Laravel wrapper requires PHP `^8.2`, Laravel Contracts/Support `^12.0 || ^13.0`, and the core package at `^2.0.0-beta.2`.
...
From your application's root, choose the environment you are using:
...
<Tabs syncKey="framework">
  <TabItem label="PHP">
    ...
    $hooks->addAction('ReportGenerated', static function (string $reportId): void {
        // Perform a synchronous side effect for this extension point.
    });

    $hooks->doAction('ReportGenerated', 'report-42');
    ```
  ...
  <TabItem label="Laravel">
    ...
    Laravel auto-discovers the wrapper. Use the `hooks()` helper for application-level calls:
...
Composer auto-discovers the Laravel service provider and facade alias. After installation, the helper is available. The [Laravel integration guide](/docs/2.x/laravel) also covers constructor injection and the optional facade.

See [Laravel integration](/docs/2.x/laravel) for container access and long-running process considerations.
