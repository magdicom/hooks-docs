...
| --- | --- | --- |
| [`addFilter()`](#method-addfilter) | Registering a value transformation. | A `RegistrationHandle`. |
| [`applyFilters()`](#method-applyfilters) | Passing a value through the filter chain. | The final value, or the original value when empty. |

<MethodReference name="addFilter" signature={`public function addFilter(
    string $hookPoint,
    array|callable $callback,
    int $priority = 10,
): RegistrationHandle`} applies="Filters" returns="A handle for this exact registration." empty="The filter is stored, but no callback runs until applyFilters() is called." exceptions="Callback and resolver exceptions surface when the filter is invoked." persistent="Yes. The callback remains registered until removed.">
  you want one module to contribute a transformation to a named value.
</MethodReference>
...
<MethodReference name="applyFilters" signature={`public function applyFilters(
    ...
    mixed $value,
    mixed ...$arguments,
): mixed`} applies="Filters" returns="The value returned by the last filter." empty="The original value is returned unchanged." exceptions="Exceptions from listeners bubble to the caller." persistent="No. It invokes the current listener snapshot.">
  you want to pass a value through ordered transformations.
</MethodReference>
