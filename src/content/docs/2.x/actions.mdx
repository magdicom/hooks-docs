...
| --- | --- | --- |
| `addAction(string $hookPoint, array\|callable $callback, int $priority = 10)` | Registering a callback for a named side effect. The callback receives the arguments passed to `doAction()`. | A `RegistrationHandle` for the exact registration. |
| `doAction(string $hookPoint, mixed ...$arguments)` | Running the registered action callbacks in priority order. | `void`; callback return values are ignored. |
