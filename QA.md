...
This acceptance record was updated on 2026-09-09 for the live site at `https://hooks.momagdi.com`. It keeps the site on Astro + Starlight, preserves all versioned routes and verified examples, and records the beta.2/beta.3 release synchronization.
...
The public robots policy now allows crawling and points to the HTTPS sitemap because the site is live. Review any future robots change together with a production acceptance pass.
...
- `magdicom/hooks` `v2.0.0-beta.2`, commit `a3e10dc8674968ea4062b924706e6ea7ed83b876`;
- `magdicom/laravel-hooks` `v2.0.0-beta.3`, commit `5003de56228c53934548a6b704f4d51ece4b9472`.
...
The manually triggered workflow remains available for approved deployments. This pass did not trigger it.
...
Public examples now use readable PascalCase hook names such as `InvoicePaid`, `InvoiceTotal`, `DashboardWidgets`, `NavigationItems`, and `CheckoutAllowed`. This is documented as a recommended convention rather than a package restriction. The released core uses exact string keys without normalization, so hook names are case-sensitive and should be treated as stable public contracts.
...
The page's order-receipt renderer example was checked against the released `Renderer::process(array $results, ProcessingContext $context): string` contract from `magdicom/hooks` `v2.0.0-beta.2`. It keeps collector results structured until one application-owned renderer escapes and formats them. `ConcatenateRenderer` remains documented as the simple built-in option: collector results are joined in order, supported scalar/string/null/Stringable values are accepted, empty results produce an empty string, and unsupported values raise `UnexpectedValueException`. The `BooleanAndProcessor` example likewise documents strict boolean results and `true` for an empty result list.
...
The guide-page explanation pass was completed on 2026-09-09. Actions, filters, collectors, processors, and renderers now introduce their methods with a purpose/return-value table and explain ordering, empty behavior, processor slots, renderer failures, and raw-versus-processed results before showing examples. This keeps the guides practical while leaving complete signatures and exhaustive method coverage to the API reference.

The homepage environment-example audit was completed on 2026-09-09. The root landing page now uses Starlight `Tabs`, `TabItem`, and `Code` components for synchronized action, filter, collector, and installation examples. PHP is the default tab and uses a `Magdicom\Hooks` instance; Laravel uses `hooks()` with its own complete example. The `/docs/2.x/` overview already had synchronized quick-start tabs and was verified against the same `framework` key and label order. The beta Composer commands remain explicit for both packages, with no global minimum-stability change.
